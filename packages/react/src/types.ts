...
type GetAbiTypeById<TAbi extends JsonAbi, Id extends number> = Extract<TAbi['types'][number], { typeId: Id }>;

type ExtractTypeFromString<TAbi extends JsonAbi, T extends string> =
  T extends `str[${infer _Length}]` ? string :
  T extends `[${infer Item}; ${infer _Length}]` ? ResolveAbiType<TAbi, GetAbiTypeById<TAbi, ExtractTypeIdFromString<Item>>>[] :
  T extends `struct ${infer Name}` ? { [K in Name]: unknown } :
  T extends `enum ${infer Name}` ? { [K in Name]: unknown } :
  T extends `(${infer Items})` ? [ExtractTypeFromString<TAbi, Items>] :
  T extends `generic ${infer _Name}` ? unknown :
  unknown;

type ResolveEnumType<TAbi extends JsonAbi, T extends JsonAbiType> = 
...
type ResolveAbiType<TAbi extends JsonAbi, T extends JsonAbiType> = 
...
export type FunctionNames<TAbi extends JsonAbi> = TAbi['functions'][number]['name'];
