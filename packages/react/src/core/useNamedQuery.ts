import { DefinedUseQueryResult, QueryKey, UseQueryOptions, UseQueryResult, useQuery } from '@tanstack/react-query';
import { useMemo } from 'react';

type ExcludeData<T> = Omit<T, 'data'>;

type NamedUseQueryResult<TName extends string, TQueryFnData = unknown, TError = unknown> = ExcludeData<UseQueryResult<TQueryFnData, TError>> & {
  [key in TName]: UseQueryResult<TQueryFnData, TError>['data'];
}

type DefinedNamedUseQueryResult<TName extends string, TQueryFnData = unknown, TError = unknown> = ExcludeData<DefinedUseQueryResult<TQueryFnData, TError>> & {
  [key in TName]: DefinedUseQueryResult<TQueryFnData, TError>['data'];
}

function createProxyHandler<TName extends string, TData = unknown, TError = unknown>(name: TName) {
...
>(
  ...
  const proxy = useMemo(() => {
    return new Proxy(query, createProxyHandler(name)) as NamedUseQueryResult<TName, TData, TError>;
  }, [name, query]);
